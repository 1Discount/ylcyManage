...
@Service
public class DistributorRoleInfoSer extends BaseService {

	private Logger logger = LogUtil.getInstance(DistributorRoleInfoSer.class);
	...
	public boolean insertinfo(RoleInfo roleInfo){
		...
		try {
			int temp=distributorRoleInfoDao.insertinfo(roleInfo);
			if(temp>0){
				logger.debug("添加角色成功");
				return true;
			}else{
	...
	public boolean editinfo(RoleInfo roleInfo){
		...
		try {
			int temp=distributorRoleInfoDao.editinfo(roleInfo);
			if(temp>0){
				logger.debug("修改角色成功");
				return true;
			}else{
	...
	public boolean deleteinfo(String mid){
		...
		try {
			int temp=distributorRoleInfoDao.delteinfo(mid);
			if(temp>0){
				logger.debug("删除角色成功");
				return true;
			}else{
	...
	public String getpageString(SearchDTO searchDTO){
		logger.debug("分页server开始");
		  try {
			  String jsonString=distributorRoleInfoDao.getpageString(searchDTO);
	...
	public List<RoleInfo> getalList(){
		logger.debug("分页server开始");
		  try {
			  return distributorRoleInfoDao.getallrole();
		} catch (Exception e) {
			logger.debug(e.getMessage());
			// TODO: handle exception
			e.printStackTrace();
