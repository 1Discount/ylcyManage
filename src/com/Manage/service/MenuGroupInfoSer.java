...
@Service
public class MenuGroupInfoSer extends BaseService {
	private Logger logger = LogUtil.getInstance(MenuGroupInfoSer.class);
	/**
	 * 添加角色组
	 * @param menuGroupInfo
	 * @return
	...
	public boolean insertinfo(MenuGroupInfo menuGroupInfo){
		...
		try {
			int temp=menuGroupInfoDao.insertinfo(menuGroupInfo);
			if(temp>0){
	...
	public boolean editinfo(MenuGroupInfo menuGroupInfo){
		...
		try {
			int temp=menuGroupInfoDao.editinfo(menuGroupInfo);
			if(temp>0){
	...
	public boolean deleteinfo(String mid){
		...
		try {
			int temp=menuGroupInfoDao.delteinfo(mid);
			if(temp>0){
	 ...
	 */
	 public String getpageString(SearchDTO searchDTO){
		  ...
		  try {
			  String jsonString=menuGroupInfoDao.getpageString(searchDTO);
