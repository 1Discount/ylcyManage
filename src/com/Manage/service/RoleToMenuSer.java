...
@Service
public class RoleToMenuSer extends BaseService {
	
	private Logger logger = LogUtil.getInstance(RoleToMenuSer.class);
	...
	public boolean batchinsert(List<RoleToMenu> list){
		...
		try {
			int temp= roleToMenuDao.batchinsert(list);
	...
	public boolean deletebyid(String rid){
		...
		try {
			int temp=roleToMenuDao.deletebyrid(rid);
	...
	public List<RoleToMenu> getbyrid(String rid){
		...
		try {
			return roleToMenuDao.getListbyrid(rid);
		} catch (Exception e) {
			// TODO: handle exception
			logger.debug(e.getMessage());
			e.printStackTrace();
