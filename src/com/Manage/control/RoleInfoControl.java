...
public class RoleInfoControl extends BaseController {
	private final static String RETURNROOT_STRING = "WEB-INF/views/admin/";
	private Logger logger = LogUtil.getInstance(RoleInfoControl.class);
	/**
	 * 跳转到添加角色界面
	 * @return
	 */
	...
	public String toadd(Model model) {
		List<MenuGroupInfo> mglist=menuInfoSer.getGroupAndMenu();
		if(mglist!=null){
	...
	public String edit(RoleInfo roleInfo,Model model) {
		...
		List<RoleToMenu> rList=roleToMenuSer.getbyrid(roleInfo.getRoleID());
		if(rList==null){
...
//			}
			...
			if(roleInfoSer.insertinfo(rInfo)){
				...
				if(roleToMenuSer.batchinsert(menus)){
	...
	public void editsave(RoleInfo roleInfo,HttpServletResponse response){
		...
		try {
			...
			//修改角色信息
			//先删除原有角色菜单映射
			if(roleInfoSer.editinfo(roleInfo) && roleToMenuSer.deletebyid(roleInfo.getRoleID())){
...
//					}
	...
	public void pagelist(SearchDTO searchDTO,RoleInfo roleInfo,
		...
		String pagesString = roleInfoSer.getpageString(seDto);
	...
	public void deletebyid(String rid,HttpServletResponse response){
		...
		try {
			if(roleInfoSer.deleteinfo(rid)){
				logger.debug("删除角色成功");
				if(roleToMenuSer.deletebyid(rid)){
