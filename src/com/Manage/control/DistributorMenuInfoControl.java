...
@Controller
@RequestMapping("/distributor/menuinfo")
public class DistributorMenuInfoControl extends BaseController {
	
	private final static String RETURNROOT_STRING = "WEB-INF/views/distributor/";
	private Logger logger = LogUtil.getInstance(DistributorMenuInfoControl.class);
	...
	public String toadd(Model model) {
		List<MenuGroupInfo> mglist=distributorMenuInfoSer.selectgroup();
		if(mglist!=null){
	...
	public void insertinfo(MenuInfo menu, Model model,
		...
		if (distributorMenuInfoSer.insertinfo(menuInfo)) {
			logger.debug("添加菜单成功");
			try {
	...
	public void editinfo(MenuInfo menu, HttpServletResponse response) {
		...
		try {
			if (distributorMenuInfoSer.editinfo(menu)) {
				logger.debug("修改菜单成功");
	...
	public void deleteinfo(String menuInfoID, HttpServletResponse response) {
		...
		try {
			if (distributorMenuInfoSer.deleteinfo(menuInfoID)) {
	...
	public void getpage(SearchDTO searchDTO, MenuInfo menu,
		...
		String pagesString = distributorMenuInfoSer.getpageString(seDto);
		try {
