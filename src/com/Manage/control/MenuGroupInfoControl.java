...
@Controller
@RequestMapping("/admin/menuGroupinfo")
/** * @author  wangbo: * @date 创建时间：2015-5-23 下午2:43:14 * @version 1.0 * @parameter  * @since  * @return  */
public class MenuGroupInfoControl extends BaseController {

	private final static String RETURNROOT_STRING = "WEB-INF/views/admin/";
	private Logger logger = LogUtil.getInstance(MenuGroupInfoControl.class);
	...
	public void insertinfo(MenuGroupInfo menuGroup, Model model,
		...
		if (menuGroupInfoSer.insertinfo(menuGroupInfo)) {
	...
	public void editinfo(MenuGroupInfo menuGroup, HttpServletResponse response) {
		...
		try {
			...
			if (menuGroupInfoSer.editinfo(menuGroup)) {
	...
	public void deleteinfo(String menuGroupID, HttpServletResponse response) {
		...
		try {
			if (menuGroupInfoSer.deleteinfo(menuGroupID)) {
	...
	public void getpage(SearchDTO searchDTO, MenuGroupInfo menuGroupInfo,
		...
		String pagesString = menuGroupInfoSer.getpageString(seDto);
		try {
