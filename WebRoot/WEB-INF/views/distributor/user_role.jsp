...
action="javascript:return;"><input type="hidden" id="user_id" value="${user.distributorID}" name="distributorID" />
<DIV class="form-group"><LABEL class="col-md-2 control-label">用户：</LABEL>
<DIV class="col-md-6">
<P class="form-control-static">${user.userName}</P>
<P class="form-control-static">${user.operatorTel}</P></DIV></DIV>
...
<DIV class="col-md-offset-2 col-md-6"><BUTTON class="btn btn-primary" onclick="subinfo();" type="submit">提交</BUTTON></DIV></DIV></FORM></DIV></DIV>
   ...
   function subinfo(){
	   ...
	   $.ajax({
			type:"POST",
			url:"<%=basePath %>customer/distributor/adduserrole",
			dataType:"html",
			data:{distributorID:$("#user_id").val(),roleID:$("#role_id").val(),roleName:rtext},
			success:function(data){
				if(data=="1"){
					bootbox.alert("角色分配成功!",function(){
						window.location="<%=basePath %>customer/distributor/index";
					});
					gridObj.refreshPage();
				}else if(data=="0"){
